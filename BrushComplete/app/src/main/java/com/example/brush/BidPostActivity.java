...
public class BidPostActivity extends AppCompatActivity {
    ...
    private DatabaseReference NewBidRef;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ProfileRef = FirebaseDatabase.getInstance().getReference().child("Users").child(current_user_id).child("Type").child("B");
        NewBidRef = FirebaseDatabase.getInstance().getReference().child("Bid");

        mToolbar = findViewById(R.id.b_page_toolbar);
        setSupportActionBar(mToolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setDisplayShowHomeEnabled(true);
        getSupportActionBar().setTitle("Post Bidding Item");

        bid_image = findViewById(R.id.bid_image);
        bid_post = findViewById(R.id.bid_post);
        bid_description = findViewById(R.id.bid_description);
        bid_money = findViewById(R.id.bid_money);
        bid_time = findViewById(R.id.bid_time);
        category_group = findViewById(R.id.category_group);
        bid_select = findViewById(R.id.bid_select);
        bid_category1 = findViewById(R.id.bid_category1);
        bid_category2 = findViewById(R.id.bid_category2);
        bid_category3 = findViewById(R.id.bid_category3);
        bid_category4 = findViewById(R.id.bid_category4);
    ...
    private void SavingPostInfoToDatabase()
    {
        ...
        if ( tempcategory.equals(digital))
        {
            dRef.child(current_user_id + postRandomName).updateChildren(bidMap);
        }
        else if ( tempcategory.equals(traditional))
        {
            tRef.child(current_user_id + postRandomName).updateChildren(bidMap);
        }
        else if ( tempcategory.equals(photography))
        {
            pRef.child(current_user_id + postRandomName).updateChildren(bidMap);
        }
        else
        {
            aRef.child(current_user_id + postRandomName).updateChildren(bidMap);
        }

        ProfileRef.child(current_user_id + postRandomName).updateChildren(bidMap);
        NewBidRef.child(current_user_id + postRandomName).updateChildren(newBidMap);
    ...
    {
        Intent mainIntent = new Intent (BidPostActivity.this,NewPostActivity.class );
